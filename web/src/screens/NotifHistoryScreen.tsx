...
const KIND_META: Record<string, { label: string; color: string }> = {
  ...
  source:   { label: 'SOURCE',   color: 'var(--accent-2)' },
  topic:    { label: 'TOPIC',    color: VIOLET },
  'ai-feed':{ label: 'AI FEED',  color: 'var(--warn)' },
  aiFeed:   { label: 'AI FEED',  color: 'var(--warn)' },
  digest:   { label: 'DIGEST',   color: 'var(--muted-2)' },
  streak:   { label: 'STREAK',   color: 'var(--success)' },
};
    ...
    <div style={{ height: '100%', overflowY: 'auto', WebkitOverflowScrolling: 'touch', background: 'var(--bg)', color: 'var(--text)', paddingBottom: 80 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 'calc(16px + env(safe-area-inset-top, 0px)) 16px 12px' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: 'var(--muted-2)', fontSize: 12, marginTop: 2 }}>{token ? `${entries.length} entries · synced from phone` : 'Pair phone to sync'}</div>
        </div>
        {token && (
          <div onClick={unpair} style={{ padding: '6px 12px', borderRadius: 999, border: '1px solid var(--line-3)', color: 'var(--muted)', fontSize: 11, fontWeight: 700, letterSpacing: 0.6, cursor: 'pointer' }}>Unpair</div>
        )}
      </div>
      ...
      {!token ? (
        <div style={{ padding: '30px 20px' }}>
          <div style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              ...
              <div style={{ color: 'var(--text)', fontSize: 17, fontWeight: 700 }}>Pair with phone</div>
            </div>
            <div style={{ color: 'var(--muted)', fontSize: 13, lineHeight: 1.55, marginBottom: 14 }}>
              On Android: <b style={{ color: 'var(--text-4)' }}>Settings → Notifications → Notification Settings → History → Pair Code</b> → tap Share → copy.
            </div>
            <input
              value={pendingToken}
              onChange={e => setPendingToken(e.target.value)}
              placeholder="Paste your pair code (ExponentPushToken[...])"
              style={{ width: '100%', boxSizing: 'border-box', padding: '12px 14px', borderRadius: 10, background: 'var(--bg)', border: `1px solid ${BORDER}`, color: 'var(--text)', fontSize: 13, fontFamily: 'monospace', outline: 'none' }}
            />
            <div onClick={savePair} style={{ marginTop: 12, padding: '11px 0', borderRadius: 10, background: pendingToken.trim() ? VIOLET : 'var(--line-2)', color: pendingToken.trim() ? 'var(--pill-text)' : 'var(--muted-3)', textAlign: 'center', fontSize: 14, fontWeight: 700, cursor: pendingToken.trim() ? 'pointer' : 'not-allowed' }}>
              Save & Sync
            </div>
            <div style={{ marginTop: 14, color: 'var(--muted-4)', fontSize: 11, lineHeight: 1.5 }}>
              Token lives in this browser only. Backend stores 200 most-recent pushes per device.
            </div>
          </div>
        </div>
      ) : loading && entries.length === 0 ? (
        <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--muted-3)' }}>Syncing…</div>
      ) : error ? (
        <div style={{ padding: '20px' }}>
          <div style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 16, color: 'var(--danger)', fontSize: 13 }}>{error}</div>
        </div>
      ) : entries.length === 0 ? (
        <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--muted-3)', fontSize: 13, lineHeight: 1.5 }}>
        ...
        <div key={sec.label} style={{ marginBottom: 16, padding: '0 16px' }}>
          <div style={{ color: 'var(--muted-3)', fontSize: 11, fontWeight: 700, letterSpacing: 1.2, marginBottom: 8, paddingLeft: 4 }}>{sec.label.toUpperCase()}</div>
                ...
                <div key={`${e.id}-${e.firedAt}`} onClick={() => openEntry(e)} style={{ display: 'flex', alignItems: 'center', padding: 12, gap: 12, borderTop: i > 0 ? '1px solid #1F1F22' : 'none', cursor: 'pointer' }}>
                  {e.imageUrl ? (
                    ...
                    <div style={{ width: 56, height: 56, borderRadius: 10, background: getArticleColor(e.id || e.headline || ''), flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.5 }}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--text)" strokeWidth="1.5"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/></svg>
                    </div>
                  )}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
                      <span style={{ color: meta.color, fontSize: 9, fontWeight: 800, letterSpacing: 0.8 }}>{meta.label}</span>
                      <span style={{ color: 'var(--muted-5)', fontSize: 9 }}>·</span>
                      <span style={{ color: 'var(--muted-3)', fontSize: 11 }}>{timeOfDay(e.firedAt)}</span>
                      {e.source && (
                        <>
                          <span style={{ color: 'var(--muted-5)', fontSize: 9 }}>·</span>
                          <span style={{ color: 'var(--muted-4)', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 110 }}>{e.source}</span>
                        </>
                      )}
                    </div>
                    <div style={{ color: 'var(--text-3)', fontSize: 13.5, fontWeight: 600, lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{e.headline ?? ''}</div>
                  </div>
                </div>
              );
            })}
