...
const CATEGORY_COLORS: Record<string, string> = {
  'India': 'var(--warn)', 'World': 'var(--info)', 'Markets': 'var(--success)',
  'Business': 'var(--topic)', 'Technology': 'var(--accent-2)',
};
...
function BellIcon({ active }: { active: boolean }) {
  return (
    <div style={{ width: 34, height: 34, borderRadius: 17, background: active ? '#1C3A6A' : 'var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background 0.2s' }}>
      {active ? (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="var(--accent-2)" stroke="var(--accent-2)" strokeWidth="1.5"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0"/></svg>
      ) : (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--muted-5)" strokeWidth="1.5"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0"/></svg>
      )}
    </div>
  );
}
    ...
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg)', WebkitOverflowScrolling: 'touch' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px', paddingTop: 'calc(16px + env(safe-area-inset-top, 0px))', position: 'sticky', top: 0, background: 'var(--bg)', zIndex: 10 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ color: 'var(--text)', fontSize: 18, fontWeight: 700 }}>Notify me about</div>
          {totalSelected > 0 && <div style={{ color: 'var(--accent-2)', fontSize: 12, fontWeight: 600, marginTop: 2 }}>{totalSelected} selected</div>}
        </div>
        <div style={{ width: 40 }} />
      </div>

      <div style={{ padding: '0 16px 40px' }}>
        <p style={{ color: 'var(--muted-4)', fontSize: 13, lineHeight: 1.5, marginBottom: 20 }}>
          Get notified when new stories arrive from your chosen topics or sources.
        </p>

        {/* Topics */}
        <div style={{ color: 'var(--muted-5)', fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginBottom: 10 }}>TOPICS</div>
        <div style={{ background: 'var(--surface)', borderRadius: 14, border: '1px solid var(--line)', marginBottom: 24, overflow: 'hidden' }}>
          {TOPIC_ITEMS.map((item, i) => {
            ...
            return (
              <div key={item.key} onClick={() => toggleFavTopic(item.key)}
                ...
                <span style={{ flex: 1, color: active ? 'var(--text-3)' : 'var(--muted-4)', fontSize: 14, fontWeight: 500, transition: 'color 0.2s' }}>{item.label}</span>
        ...
        {/* Sources */}
        <div style={{ color: 'var(--muted-5)', fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginBottom: 10 }}>SOURCES</div>
        {SOURCE_CATEGORIES.map(cat => {
          const accentColor = CATEGORY_COLORS[cat.label] ?? 'var(--accent-2)';
          return (
            <div key={cat.label} style={{ background: 'var(--surface)', borderRadius: 14, border: '1px solid var(--line)', marginBottom: 16, overflow: 'hidden' }}>
              ...
              {cat.sources.map((src) => {
                ...
                return (
                  <div key={src} onClick={() => toggleFavSource(src)}
                    style={{ display: 'flex', alignItems: 'center', padding: '10px 16px', gap: 12, borderTop: '1px solid var(--line)', cursor: 'pointer' }}>
                    <div style={{ width: 28, height: 28, borderRadius: 7, overflow: 'hidden', background: 'var(--line)', flexShrink: 0 }}>
                      <img src={faviconUrl(src)} alt={src} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                    <span style={{ flex: 1, color: active ? 'var(--text-3)' : 'var(--muted-4)', fontSize: 14, fontWeight: 500, transition: 'color 0.2s' }}>{src}</span>
                    <BellIcon active={active} />
                  </div>
                );
              })}
