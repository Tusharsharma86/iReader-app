  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg)', WebkitOverflowScrolling: 'touch' }}>
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: 'var(--text)', fontSize: 22, fontWeight: 800 }}>Topic Interests</div>
          <div style={{ color: 'var(--muted-3)', fontSize: 12, marginTop: 2 }}>
            {totalStarred > 0
      ...
      <div style={{
        ...
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--muted-4)" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
        <input
          value={query}
          ...
          style={{
            ...
            color: 'var(--text)', fontSize: 14, padding: 0,
          }}
        />
        {query.length > 0 && (
          <button onClick={() => setQuery('')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="var(--muted-4)"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15" stroke="var(--surface-3)" strokeWidth="2"/><line x1="9" y1="9" x2="15" y2="15" stroke="var(--surface-3)" strokeWidth="2"/></svg>
          </button>
        )}
      </div>
      ...
      <div style={{ padding: '0 16px 48px' }}>
        {grouped.map(group => (
          <div key={group.category} style={{ marginBottom: 18 }}>
            <div style={{ color: 'var(--muted-3)', fontSize: 11, fontWeight: 700, letterSpacing: 1.2, marginBottom: 8, marginLeft: 4 }}>
              {group.category.toUpperCase()}
            </div>
            <div style={{ background: 'var(--surface-3)', borderRadius: 14, overflow: 'hidden' }}>
              {group.items.map((t, i) => (
                <div key={t.id} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{ color: 'var(--text-2)', fontSize: 14, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.label}</span>
                  </div>
                  <StarRow
